...
from utils.vrepConst import *  # noqa: F403
...
try:
    if platform.system() == "cli" or platform.system() == "Windows":
        dll_path = os.path.join(current_dir, "remoteApi.dll")
        libsimx = ct.CDLL(dll_path)
    elif platform.system() == "Darwin":
        dylib_path = os.path.join(current_dir, "remoteApi.dylib")
        libsimx = ct.CDLL(dylib_path)
    else:  # Linux
        so_path = os.path.join(current_dir, "remoteApi.so")
        libsimx = ct.CDLL(so_path)
except Exception as e:
    print(f"Error loading remote API library: {e}")
